...
var articles = {
	...
	'week4': {
		...
		html: (
			React.DOM.div(null, 
				...
				React.DOM.figure( {class:"img"}, 
					React.DOM.img( {src:"patterns/setup.jpg"})
				),React.DOM.figcaption(null, "My set up for this weeks recording session. The third mic is hidden behind the camera."),
				React.DOM.p(null, 
	...
	'week5' : {
		...
		html: (
			React.DOM.div(null, 
				" /* Notes on SOS: What's The Frequency */ ",
				SubHeader( {text:"Frequency Freakout (So, You Think You Can EQ?)"}),
				React.DOM.p(null, 
					"EQ is the meticulous process of manipulating the frequency of an audio source. Anything between 0 and 20 kHz could be both dampened and amplified by EQ. Maybe you want to make that harmonic really ring out above everything else, or for your bass to sound brighter. Or perhaps, you really dislike the static scratchiness in the 14 kHz neighborhood. Good news, those can be cut or softened all with a simple application of equalization. Achievement is only a few filters away. Lets begin with a pretty recognizable example.",
					"EQ - Low pass filter is powered by capacitors and cuts out the higher frequency levels. High pass filters act conversely, and are wired through resistors. In either case the resistor/capacitor is in series, and its cousin is sinking to ground. "
				)
